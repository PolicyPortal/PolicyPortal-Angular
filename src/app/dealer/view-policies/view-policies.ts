import { Component, signal, ViewChild, AfterViewInit, ChangeDetectorRef, computed, OnInit } from '@angular/core';
...
// Define an interface for type-safety (ADJUSTED FOR BACKEND DATA)
...
export class ViewPolicies implements OnInit {
  ...
  loadPolicies(): void {
    ...
    this.policyService.getPoliciesByDealer(this.authService.getDealerId()).pipe(
      catchError(err => {
        console.error('Failed to load policies:', err);
        this.errorMessage.set('Failed to load policies. Please try again later.');
        return of([] as Policy[]); // Return empty array on error
      })
    ).subscribe(data => {
